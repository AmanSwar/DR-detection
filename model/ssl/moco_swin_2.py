...
class MoCoV3Model(nn.Module):
    def __init__(self, base_model='swin_tiny_patch4_window7_224', projection_dim=256, hidden_dim=512,
                 queue_size=65536, momentum=0.99, pretrained=False):
        super(MoCoV3Model, self).__init__()
        self.m = momentum

        # Swin Transformer backbone with image size 256x256
        self.query_encoder = timm.create_model(
            base_model, 
            pretrained=pretrained, 
            num_classes=0,
            img_size=256  # Critical change for fundus images
        )
        ...
        self.key_encoder = timm.create_model(
            ...
            pretrained=pretrained, 
            num_classes=0,
            img_size=256  # Matching query encoder
        )
...
# Training adjustments in main function
def main():
    config = {
        "epochs": 400,  # Increased for better convergence
        "batch_size": 256,  # Larger batch size for 100k images
        "lr": 3e-4,  # Adjusted learning rate
        "lr_min": 1e-6,
        "warm_up_epochs": 20,
        "temperature": 0.2,
        "momentum": 0.99,
        "queue_size": 65536,  # Larger memory bank
    ...
    # Initialize model with Swin backbone
    model = MoCoV3Model(
...
# Keep other components (loss, validation, etc.) unchanged