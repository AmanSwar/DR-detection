...
class EnhancedDRClassifier(nn.Module):
    ...
    def forward(self, x, get_attention=False, update_prototypes=False, labels=None):
        ...
        if get_attention:
            return logits, grade_probs, h, attended_features
        return logits, grade_probs


def OrdinalDomainLoss(outputs, labels, grade_outputs=None, lambda_consistency=0.1):
...
def train_one_epoch(model, dataloader, optimizer, device, epoch, wandb_run, scaler=None, 
                    lambda_consistency=0.1, scheduler=None):
    model.train()
    running_loss = 0.0
    all_labels = []
    all_preds = []
    ...
    for i, (images, labels, _) in enumerate(dataloader):
        ...
        if scaler is not None:
            with autocast():
                logits, grade_probs = model(images, update_prototypes=True, labels=labels)
...
def validate(model, dataloader, device, epoch, wandb_run, lambda_consistency=0.1):
    ...
    with torch.no_grad():
        for images, labels in dataloader:
            ...
            logits, grade_probs = model(images)
            loss = OrdinalDomainLoss(
                logits, labels, 
                grade_outputs=grade_probs, 
                lambda_consistency=lambda_consistency
...
def main():
    ...
    checkpoint_dir = "chckpt/finetune_nofreeze/no_init_fine_3"
