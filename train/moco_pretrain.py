...
class MoCoV3Model(nn.Module):
    def __init__(
        ...
        self.query_encoder = timm.create_model(
            base_model, pretrained=pretrained, num_classes=0
        )
        ...
        self.key_encoder = timm.create_model(
            base_model, pretrained=pretrained, num_classes=0
        )
...
def train_one_epoch(
    model, dataloader, optimizer, scheduler, temperature, device, epoch, wandb_run
):
    model.train()
    running_loss = 0.0
    for i, (im_q, im_k) in enumerate(dataloader):
        ...
        if i % 10 == 0:
            ...
            logging.info(
                f"Epoch [{epoch+1}] Step [{i}/{len(dataloader)}] Loss: {loss.item():.4f} LR: {current_lr:.6f}"
            )
            wandb_run.log(
                {
...
def main():
    ...
    if os.path.exists(checkpoint_path):
        ...
        logging.info(
            f"Loaded checkpoint from epoch {start_epoch}, resuming training from epoch {start_epoch}"
        )
    else:
    ...
    try:
        for epoch in range(start_epoch, config["epochs"]):
            ...
            val_loss = validate(
                model, valid_loader, temperature, device, epoch, wandb_run
            )
            ...
            save_checkpoint(
                checkpoint_state, "model/new/chckpt/moco/new/new", epoch_ckpt
            )
            ...
            if val_loss < best_val_loss:
                ...
                save_checkpoint(
                    checkpoint_state,
                    "model/new/chckpt/moco/new/new",
                    "best_checkpoint.pth",
                )

            if (epoch + 1) % 3 == 0:
                linear_probe_evaluation(
                    model, probe_train_loader, probe_val_loader, device, wandb_run
                )
                knn_evaluation(
                    model,
                    probe_train_loader,
        ...
        save_checkpoint(
            checkpoint_state,
            "model/new/chckpt/moco/new/new",
            f"interrupt_checkpoint_epoch_{epoch+1}.pth",
        )
    finally:
        wandb_run.finish()
