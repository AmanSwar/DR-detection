...
class MoCoV3Model(nn.Module):
    def __init__(self, base_model='swin_base_patch4_window7_224', projection_dim=256, hidden_dim=512,
                 queue_size=16384, momentum=0.99, pretrained=False, model_kwargs={}):
        super(MoCoV3Model, self).__init__()
        self.m = momentum

        # Build the query encoder with SwinTransformer backbone + projection head
        self.query_encoder = timm.create_model(base_model, pretrained=pretrained, num_classes=0, **model_kwargs)
        ...
        self.key_encoder = timm.create_model(base_model, pretrained=pretrained, num_classes=0, **model_kwargs)
...
def main():
    config = {
        ...
        "temperature": 0.2,
        "momentum": 0.99,
        "queue_size": 16384,  # Increased for 100,000 images
        "base_model": "swin_base_patch4_window7_224",
        "projection_dim": 256,
        "hidden_dim": 512,
        "pretrained": False,
        "model_kwargs": {"img_size": 256, "window_size": 8}  # For SwinTransformer
    }
